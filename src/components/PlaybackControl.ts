...
export class PlaybackControl {
  ...
  constructor() {
    ...
    this.element.innerHTML = `
      <button class="playback-toggle" title="${t('playback.toggleMode')}">
        <span class="playback-icon">⏪</span>
      </button>
      <div class="playback-panel hidden">
        <div class="playback-header">
          <span>${t('playback.historicalPlayback')}</span>
        ...
        <div class="playback-slider-container">
          ...
          <div class="playback-time">${t('common.live')}</div>
        </div>
        <div class="playback-controls">
          <button class="playback-btn" data-action="start">⏮</button>
          <button class="playback-btn" data-action="prev">◀</button>
          <button class="playback-btn playback-live" data-action="live">${t('common.live')}</button>
  ...
  private updateTimeDisplay(): void {
    ...
    if (!this.isPlaybackMode || this.timestamps.length === 0) {
      display.textContent = t('common.live');
