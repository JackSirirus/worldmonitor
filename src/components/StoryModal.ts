...
export function openStoryModal(data: StoryData): void {
  ...
  modalEl.innerHTML = `
    <div class="story-modal">
      <button class="story-close-x" aria-label="${t('modal.close')}">
      ...
      <div class="story-modal-content">
        <div class="story-loading">
          ...
          <span>${t('storyModal.generatingStory')}</span>
        </div>
      </div>
      <div class="story-share-bar" style="display:none">
        <button class="story-share-btn story-save" title="${t('storyModal.savePng')}">
        ...
        <button class="story-share-btn story-whatsapp" title="${t('storyModal.whatsapp')}">
          ...
          <span>${t('storyModal.whatsapp')}</span>
        </button>
        <button class="story-share-btn story-twitter" title="${t('storyModal.x')}">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
          <span>${t('storyModal.x')}</span>
        </button>
        <button class="story-share-btn story-linkedin" title="${t('storyModal.linkedIn')}">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
          <span>${t('storyModal.linkedIn')}</span>
        </button>
        <button class="story-share-btn story-copy" title="${t('storyModal.link')}">
  ...
  requestAnimationFrame(async () => {
    ...
    try {
      ...
      if (content) content.innerHTML = `<div class="story-error">${t('storyModal.failedToGenerate')}</div>`;
    }
  });
}
...
function downloadStory(): void {
  ...
  a.click();
  flashButton('.story-save', t('storyModal.saved'), 'Save');
}
...
async function shareWhatsApp(data: StoryData): Promise<void> {
  ...
  try {
    ...
    flashButton('.story-whatsapp', t('storyModal.copied'), 'WhatsApp');
  } catch {
    downloadStory();
    flashButton('.story-whatsapp', t('storyModal.saved'), 'WhatsApp');
  }
...
function flashButton(selector: string, flashText: string, originalText: string): void {
  ...
  if (span) {
    // Map original text to translation keys
    const translationKeys: Record<string, string> = {
      'Save': t('modal.save'),
      'WhatsApp': t('storyModal.whatsapp'),
      'X': t('storyModal.x'),
      'LinkedIn': t('storyModal.linkedIn'),
      'Link': t('storyModal.link'),
    };
    const translatedOriginal = translationKeys[originalText] || originalText;
    const translatedFlash: Record<string, string> = {
      'Saved!': t('storyModal.saved'),
      'Copied!': t('storyModal.copied'),
      'Opening...': t('storyModal.opening'),
    };
    span.textContent = translatedFlash[flashText] || flashText;
    setTimeout(() => { if (span) span.textContent = translatedOriginal; }, 2500);
  }
}
