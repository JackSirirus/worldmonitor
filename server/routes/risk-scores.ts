...
async function fetchACLEDProtests(): Promise<AcledEvent[]> {
  try {
    ...
    if ((data as any).message) {
      console.warn('[RiskScores] ACLED API returned message:', (data as any).message);
      throw new Error((data as any).message);
    }
    if ((data as any).error || (data as any).success === false) {
      console.warn('[RiskScores] ACLED API returned error:', (data as any).error || 'unknown');
      throw new Error((data as any).error || 'ACLED API error');
    }
...
function computeCIIScores(protests: AcledEvent[]) {
  const countryEvents = new Map<string, CountryEvents>();
...
function computeStrategicRisk(ciiScores: any[]) {
...
router.get('/', async (req, res) => {
  ...
  if (redisClient) {
    try {
      ...
      if (cached && typeof cached === 'object') {
        ...
        recordCacheTelemetry('/api/risk-scores', 'REDIS-HIT');
  ...
  try {
    ...
    recordCacheTelemetry('/api/risk-scores', 'MISS');
    return res.status(200)
      .set({
        'Content-Type': 'application/json',
        'Cache-Control': 'public, max-age=300',
    ...
    if (redisClient) {
      try {
        ...
        if (stale && typeof stale === 'object') {
          ...
          recordCacheTelemetry('/api/risk-scores', 'STALE');
          return res.status(200)
            .set({
              'Content-Type': 'application/json',
    ...
    recordCacheTelemetry('/api/risk-scores', 'BASELINE');
    return res.status(200)
      .set({
        'Content-Type': 'application/json',
        'Cache-Control': 'public, max-age=60',
